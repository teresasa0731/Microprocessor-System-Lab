...
;------------------------------------------------------------
;	./src/oled_i2c.c:61: void OLED_DisplayChar(uint8_t ch)
;	-----------------------------------------
...
;	./src/oled_i2c.c:66: if(ch!='\n') {  /* TODO */ 
	...
	ret
00124$:
;	./src/oled_i2c.c:67: index = (ch - 0x20);
	mov	r6,#0x00
	mov	a,r7
	add	a,#0xe0
...
;	./src/oled_i2c.c:68: index = index * 5; // As the lookup table starts from Space(0x20)
	mov	dptr,#0x0005
	lcall	__mulint
	mov	r6,dpl
	mov	r7,dph
;	./src/oled_i2c.c:70: for(i = 0; i < 5; i ++)
	mov	r5,#0x00
00104$:
;	./src/oled_i2c.c:71: oledSendData(OledFontTable[index + i]); /* Get the data to be displayed for LookUptable*/
	mov	ar3,r5
	mov	r4,#0x00
	mov	a,r3
	add	a,r6
...
00125$:
	jc	00104$
;	./src/oled_i2c.c:73: oledSendData(0x00); /* Display the data and keep track of cursor */
	mov	dpl,#0x00
;	./src/oled_i2c.c:75: }
	ljmp	_oledSendData
;------------------------------------------------------------
...
;------------------------------------------------------------
;	./src/oled_i2c.c:95: void OLED_DisplayString(uint8_t *ptr)
;	-----------------------------------------
...
00101$:
	...
	mov	r4,a
	jz	00104$
;	./src/oled_i2c.c:98: OLED_DisplayChar(*ptr++);
	mov	dpl,r4
	inc	r5
	cjne	r5,#0x00,00116$
	inc	r6
...
;------------------------------------------------------------
;	./src/oled_i2c.c:111: void OLED_Clear(void)
;	-----------------------------------------
;	 function OLED_Clear
;	-----------------------------------------
_OLED_Clear:
;	./src/oled_i2c.c:114: for(oled_clean_page = 0 ; oled_clean_page < 8 ; oled_clean_page ++) {
	mov	r7,#0x00
00105$:
;	./src/oled_i2c.c:115: OLED_SetCursor(oled_clean_page,0);
	mov	_OLED_SetCursor_PARM_2,#0x00
	mov	dpl,r7
	push	ar7
	lcall	_OLED_SetCursor
	pop	ar7
;	./src/oled_i2c.c:116: for(oled_clean_col= 0 ; oled_clean_col < 128 ; oled_clean_col ++) {
	mov	r6,#0x00
00103$:
;	./src/oled_i2c.c:117: oledSendData(0);
	mov	dpl,#0x00
	push	ar7
	push	ar6
	lcall	_oledSendData
...
;------------------------------------------------------------
;	./src/oled_i2c.c:136: void OLED_SetCursor(uint8_t lineNumber,uint8_t cursorPosition)
;	-----------------------------------------
;	 function OLED_SetCursor
;	-----------------------------------------
_OLED_SetCursor:
	mov	r7,dpl
;	./src/oled_i2c.c:138: cursorPosition = cursorPosition + 2;
	mov	r6,_OLED_SetCursor_PARM_2
	mov	a,#0x02
	add	a,r6
	mov	_OLED_SetCursor_PARM_2,a
;	./src/oled_i2c.c:139: oledSendCommand(0x0f&cursorPosition);
	mov	r6,_OLED_SetCursor_PARM_2
	mov	a,#0x0f
	anl	a,r6
	mov	dpl,a
	...
	lcall	_oledSendCommand
	pop	ar6
;	./src/oled_i2c.c:140: oledSendCommand(0x10|(cursorPosition>>4));
	mov	a,r6
	swap	a
	anl	a,#0x0f
	mov	r6,a
	...
	lcall	_oledSendCommand
	pop	ar7
;	./src/oled_i2c.c:141: oledSendCommand(0xb0|lineNumber);
	mov	a,#0xb0
	orl	a,r7
	mov	dpl,a
;	./src/oled_i2c.c:142: }
...
;------------------------------------------------------------
;	./src/oled_i2c.c:147: void oledSendCommand(uint8_t cmd)
;	-----------------------------------------
;	 function oledSendCommand
;	-----------------------------------------
_oledSendCommand:
	mov	_Single_WriteI2C_PARM_3,dpl
;	./src/oled_i2c.c:149: Single_WriteI2C(OLED_SlaveAddress, SSD1306_COMMAND, cmd);
...
;cmd                       Allocated to registers 
;------------------------------------------------------------
;	./src/oled_i2c.c:152: void oledSendData(uint8_t cmd)
;	-----------------------------------------
;	 function oledSendData
;	-----------------------------------------
_oledSendData:
	mov	_Single_WriteI2C_PARM_3,dpl
;	./src/oled_i2c.c:154: Single_WriteI2C(OLED_SlaveAddress, SSD1306_DATA_CONTINUE, cmd);
